<template>
  <div class="animal" :style="{ height: height + 'px' }">
    ...
    <div v-else-if="scene === 6" class="scene flex" @click="changeScene(7)">
      ...
      <div class="text center">
        胜利{{ winPlayers.length }}人，淘汰{{ losePlayers.length }}人
      </div>
    </div>
    ...
    <div v-else-if="scene === 9" class="scene flex" @click="changeScene(10)">
      ...
      <div class="text center">
        在大家都完全随机出牌的情况下，平均有一半的人获胜，剩下一半的人被淘汰？
      </div>
    </div>

    <div v-else-if="scene === 10" class="scene flex" @click="changeScene(11)">
      <div class="text center mb">
        我已经做过一次实验，1000人局进行了10次游戏，最终的结果是：
      </div>
      ...
      <div class="text center">
        实际上胜利者会比淘汰者要多一些，胜率大约为53.99%
      </div>
    </div>

    <div v-else-if="scene === 11" class="scene flex" @click="changeScene(12)">
      <div class="text center mb">
        电影当中还有一块电子屏，上面记录了当前还存活的玩家手中3种牌的总数，就是下面这个东西：
      </div>
    ...
    <div v-else-if="scene === 12" class="scene flex" @click="changeScene(13)">
      <div class="text center mb">
        现在出现了一名特殊的玩家，他叫观察者，名字用蓝色来表示，他的出牌策略不同于随机者，他会根据电子屏上显示的牌数，<span
          class="bold"
          >他总会尽量打出一张当前电子屏上最多的那张牌的克星牌</span
        >
      </div>
      <one-player
        style="margin-bottom: 20px"
      ...
      <div class="text center">
        举个栗子：比如现在石头的数量最多，那么这名观察者就会打出布，如果没有布，那么他会打出第二多的布的克星牌，即剪刀，如果剪刀也没有，那么只能打出石头了
      </div>
    </div>
    ...
    <div v-else-if="scene === 13" class="scene flex" @click="changeScene(14)">
      ...
      <div class="text center">
        如果剪刀和布都是最多的，那么他就会从石头和剪刀中随机选一个出一张牌，如果其中一个没有牌就打另一个，如果两个都没有，那只能打布了
      </div>
    </div>

    <div v-else-if="scene === 14" class="scene flex" @click="changeScene(15)">
      <div class="text center mb">
        好，接下来我们再模拟一次50人的游戏，这次我们加入25位观察者，看看结果如何
      </div>
      <div class="players">
        <one-player
          v-for="(p, index) in players"
    ...
    <div v-else-if="scene === 15" class="scene flex" @click="changeScene(16)">
      ...
      <div class="text center">
        其中观察者胜利{{
          winPlayers.filter((p) => p.style === "observe").length
        }}人，淘汰{{
          losePlayers.filter((p) => p.style === "observe").length
        }}人
      </div>
    </div>
    ...
    <div v-else-if="scene === 16" class="scene flex" @click="changeScene(17)">
      <div class="text center mb">
        看起来观察者并没有想象中的那么厉害是吗？我又做了一次实验，1000人的游戏，包含500个随机者和500个观察者，一共进行10次游戏，结果如下：
      </div>
      <table class="table mb">
        <tr>
          <td>次数</td>
      ...
      <div class="text center">
        是不是感到一点点失望，随机者的胜者比例是53.99%，而观察者的为54.48%，仅比随机者多了0.5%还不到
      </div>
    </div>

    <div v-else-if="scene === 17" class="scene flex" @click="changeScene(18)">
      <div class="text center mb">
        但是不要失望，不要觉得观察者就是个憨批，我们暂时先把观察者放一边
      </div>
      <div class="text center mb">
        接下来我们再引入一种新的角色，他叫和平者，名字用紫色来表示
      </div>
      <one-player
        style="margin-bottom: 20px"
        :player="defaultPlayer"
      ></one-player>
      <div class="text center">
        和平者是一群佛系玩家，他们不求获胜，只求平局，因此他们的出牌策略是<span
          class="bold"
          >尽量打出一张当前电子屏上最多的那张牌</span
        >
      </div>
    </div>
    ...
    <div v-else-if="scene === 18" class="scene flex" @click="changeScene(19)">
      <div class="text center mb">
        我们再模拟一次50人的游戏，25位随机者，25位和平者，看看结果如何
      </div>
      <div class="players">
        <one-player
          v-for="(p, index) in players"
    ...
    <div v-else-if="scene === 19" class="scene flex" @click="changeScene(20)">
      ...
      <div class="text center">
        其中和平者胜利{{
          winPlayers.filter((p) => p.style === "peace").length
        }}人，淘汰{{ losePlayers.filter((p) => p.style === "peace").length }}人
      </div>
    </div>

    <div v-else-if="scene === 20" class="scene flex" @click="changeScene(21)">
      <div class="text center mb">
        同理我又做了实验，1000人的游戏，包含500个随机者和500个和平者，一共进行10次游戏，结果如下：
      </div>
      <table class="table mb">
        <tr>
          <td>次数</td>
    ...
    <div v-else-if="scene === 21" class="scene flex" @click="changeScene(22)">
      <div class="text center mb">
        至此为止我们已经有3种出牌策略的玩家，分别是随机者、观察者、和平者，目前为止他们三者的胜率几乎相同
      </div>
      <div class="text center mb">
        接下来我们引进第四种玩家：欺诈者，用红色的名字来表示。
      </div>
      <one-player
        style="margin-bottom: 20px"
        :player="defaultPlayer"
      ></one-player>
      <div class="text center mb">
        欺诈者在每一次出牌前会向对方宣布自己接下来要出的牌，比如欺诈者宣称自己将要出布，此时如果对阵的是一名观察者，那么他就会暂时无视数据板上的数字，出牌的优先级是剪刀>布>石头；同理，如果对阵的是和平者，和平者的出牌优先级是布>剪刀>石头。然而欺诈者将会打出的牌不是布而是剪刀
      </div>
      <div class="text center bold">
        实际上，欺诈者的出牌策略和随机者完全相同，唯一不同的就是他会根据自己即将打出的牌，说一句欺诈对手的话，从而影响对手的出牌策略
      </div>
    </div>

    <div v-else-if="scene === 22" class="scene flex" @click="changeScene(23)">
      <div class="text center mb">
        接下来我们要模拟一次大规模的战斗，20个随机者，10个观察者，10个和平者，10个欺诈者，群魔乱舞，看看结果如何
      </div>
      <div class="players">
        <one-player
          v-for="(p, index) in players"
    ...
    <div v-else-if="scene === 23" class="scene flex" @click="changeScene(24)">
      ...
      <div class="text center">
        随机者胜利{{
          winPlayers.filter((p) => p.style === "random").length
        }}人，淘汰{{ losePlayers.filter((p) => p.style === "random").length }}人
      </div>
      <div class="text center">
        观察者胜利{{
          winPlayers.filter((p) => p.style === "observe").length
      ...
      <div class="text center">
        欺诈者胜利{{
          winPlayers.filter((p) => p.style === "cheat").length
        }}人，淘汰{{ losePlayers.filter((p) => p.style === "cheat").length }}人
      </div>
    </div>

    <div v-else-if="scene === 24" class="scene flex" @click="changeScene(25)">
      <div class="text center mb">
        是不是看到了一些不同？老规矩，扩大实验人数，总1000人，400随机者，200观察者，200和平者，200欺诈者，10局游戏，结果如下：
      </div>
    ...
    <div v-else-if="scene === 25" class="scene flex" @click="changeScene(26)">
      ...
      <div class="text center mb">
        观察者的胜率由之前的54%左右提升到了67.80%
      </div>
      <div class="text center mb">和平者就很惨了，胜率仅有11.00%</div>
      <div class="text center">
        最新加入的欺诈者以78.75%的胜率获得了胜率排行第一
      </div>
    </div>

    <div v-else-if="scene === 26" class="scene flex" @click="changeScene(27)">
      <div class="text center mb">
        我们再引入一个角色：干扰者，名字用黄色表示
      </div>
      <one-player
        style="margin-bottom: 20px"
        :player="defaultPlayer"
      ></one-player>
      <div class="text center mb bold">
        干扰者的本质是随机者，而且是一群追求快乐的随机者，干扰者的出牌策略与随机者相同，并且在出牌前会宣布一张牌，与欺诈者不同的是，过他宣布的牌与他要打的牌没有任何关系，是完全随机宣布，也完全随机出牌
      </div>
      <div class="text center">
        接下来我们再模拟一场简单的游戏，5种角色各10人，看看结果如何
      </div>
    </div>
    ...
    <div v-else-if="scene === 27" class="scene flex" @click="changeScene(28)">
      ...
      <div class="text center">
        干扰者胜利{{
          winPlayers.filter((p) => p.style === "interfere").length
        }}人，淘汰{{
          losePlayers.filter((p) => p.style === "interfere").length
        }}人
      </div>
    </div>
    ...
    <div v-else-if="scene === 28" class="scene flex" @click="changeScene(29)">
      <div class="text center mb">
        老规矩，扩大实验人数，总1000人，5种角色各200人，10局游戏，结果如下：
      </div>
      <table class="table small-font">
        <tr>
          <td>次数</td>
    ...
    <div v-else-if="scene === 29" class="scene flex" @click="changeScene(100)">
      ...
      <div class="text center">
        值得注意的是除了欺诈者，随机者、观察者、和平者的胜率都比没有干扰者的时候高了一点点
      </div>
    </div>

    <div v-else-if="scene === 100" class="scene flex" @click="changeScene(101)">
      <div class="text center">
        接下来你将亲自进入一场游戏的模拟，游戏一共由20名玩家组成，包括你和19个AI，AI是上述随机一种风格，你需要自己去判断他们是哪种类型，并且在一定时间内获胜，祝你好运
      </div>
    </div>
    ...
    <div v-else-if="scene === 666" class="scene flex"></div>

    <div v-if="battleIndex !== -1" class="mask">
      <div class="dialog">
        <div class="row">vs {{ players[battleIndex].name }}</div>
        <div v-if="me.opponentPromise !== null" class="row">
          对方宣称即将打出{{ ["石头", "剪刀", "布"][me.opponentPromise] }}
        </div>
        <el-form>
        ...
        <div
          v-if="players[battleIndex].status === 'playing' && me.willPlay !== ''"
          class="btn"
          @click="battle()"
        >
          出牌
        ...
        <div
          v-if="players[battleIndex].status !== 'playing'"
          class="btn"
          @click="battleIndex = -1"
        >
          对手已离开游戏，关闭
    ...
    switch (this.style) {
      ...
      case "observe":
        ...
        for (let i = 0; i < allSort.length; i++) {
          let arr = allSort[i].filter(
            (index) =>
              this[["rock", "scissors", "paper"][(index - 1 + 3) % 3]] > 0
          );
          if (arr.length > 0) {
      ...
      case "peace":
        ...
        for (let i = 0; i < allSort.length; i++) {
          let arr = allSort[i].filter(
            (index) => this[["rock", "scissors", "paper"][index]] > 0
          );
          if (arr.length > 0) {
            play = getRandom(arr);
            break;
...
export default {
  ...
  methods: {
      ...
      if (loop) {
        ...
        this.timeInterval = setTimeout(() => {
          if (this.playingPlayers.length >= 2) {
            ...
            if (promiseA !== null) {
              log += `${playerA.name}宣称要打${
                ["石头", "剪刀", "布"][promiseA]
              }，`;
            }
            if (promiseB !== null) {
              log += `${playerB.name}宣称要打${
                ["石头", "剪刀", "布"][promiseB]
              }，`;
            }
            log += `对战结果：`;
            switch ((playA - playB + 3) % 3) {
              case 0:
                log += `${playerA.name}出${["石头", "剪刀", "布"][playA]}，${
                  playerB.name
                }出${["石头", "剪刀", "布"][playB]}，平手`;
                break;
              case 1:
                ...
                log += `${playerA.name}出${["石头", "剪刀", "布"][playA]}，${
                  playerB.name
                }出${["石头", "剪刀", "布"][playB]}，${playerB.name}胜`;
                break;
              case 2:
                playerA.star++;
                playerB.star--;
                log += `${playerA.name}出${["石头", "剪刀", "布"][playA]}，${
                  playerB.name
                }出${["石头", "剪刀", "布"][playB]}，${playerA.name}胜`;
                break;
            }
            this.logs.push(log);
            this.toBottom("logs");
      ...
      for (let i = 0; i < numberOfAI; i++) {
        this.players.push(
          new Player(
            i,
            "第" + (i + 1) + "号电脑",
            getRandom(["random", "observe", "peace", "cheat", "interfere"])
          )
        );
      }
      this.me.play = true;
    ...
    battle() {
      ...
      switch ((playA - playB + 3) % 3) {
        case 0:
          log += `你出${["石头", "剪刀", "布"][playA]}，${playerB.name}出${
            ["石头", "剪刀", "布"][playB]
          }，平手`;
          this.notify("平手");
          break;
    ...
    judge() {
      if (
        this.me.rock + this.me.scissors + this.me.paper === 0 &&
        this.me.star >= 3
      ) {
        // win
        this.battleIndex = -1;
